...
/**
 * The one deliberately-loud moment: a transient top-of-screen reveal card + a confetti burst,
 * fired when a genuinely-new achievement-unlock or carpool-match notification lands (see
 * useCelebrationWatcher). Motion + confetti — a distinct visual language from the glass surfaces.
 * Mounted once by AppLayout. Auto-dismisses; tap / ✕ / Esc to close.
 */
export function CelebrationOverlay() {
  ...
  const subtitle = isAchievement ? celebration.notification.body : 'A carpool matches your plans. Tap to see the ride.';
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[70] flex justify-center px-4 pt-[calc(env(safe-area-inset-top)+0.75rem)]" role="status" aria-live="polite">
      <div
        className={cn(
          'pointer-events-auto relative w-full max-w-md overflow-hidden rounded-3xl border p-4 shadow-elevation-3 animate-pop-in cursor-pointer',
      ...
      >
        ...
        <div className="flex items-center gap-4">
          <span className={cn('grid h-16 w-16 shrink-0 place-items-center rounded-2xl', isAchievement ? tier.badge : 'bg-brand/20 text-brand-strong')}>
            {isAchievement ? <Icon name={meta.icon || 'Trophy'} className="h-8 w-8" strokeWidth={1.75} /> : <PartyPopper className="h-8 w-8" strokeWidth={1.75} />}
          </span>
