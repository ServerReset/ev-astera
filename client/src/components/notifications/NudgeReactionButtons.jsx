...
// Emoji reaction pack — each maps to a nudgeReactSchema enum value (shared/validation.js) and a
// nudge_reaction_* notification template. The emoji carries the meaning ("on my way" / "almost
// done") so the reply says something real instead of a bare up/down.
const REACTIONS = [
    ...
    <div className="flex flex-wrap items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
      {REACTIONS.map((r) => {
        ...
        return (
          ...
          >
            <span aria-hidden="true" className={cn('block transition-transform', active && 'animate-pop-in')}>{r.emoji}</span>
          </button>
        );
      })}
    </div>
