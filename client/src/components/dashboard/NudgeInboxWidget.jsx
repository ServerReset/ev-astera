 ...
 * Push enabled → one compact row (the OS push already delivered the signal). Push disabled →
 * a richer inbox with inline nudge reactions.
 */
    ...
    <Card>
      ...
      <ul className="stagger space-y-1.5">
        {visible.map((n) => {
          ...
          return (
            <li key={n.id}>
              {/* Nudge rows nest NudgeReactionButtons' own <button>s — a <button> wrapper here
                  would be invalid (nested interactive), so this is a div with button semantics. */}
              <div
                role="button"
                tabIndex={0}
                onClick={() => open(n)}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {!subscribed && n.type === NOTIFICATION_TYPES.NUDGE && (
                    <div className="mt-1">
